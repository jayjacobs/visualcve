...
fullplot <- map_dfr(alldates, function(curdate) {
  has_cwe %>% 
    filter(published > (curdate - days(180)),
           published <= curdate) %>% 
    summarize(x = sum(cwe), n=n(), pct = x/n, date = curdate)
})

fullplot2 <- map_dfr(alldates, function(curdate) {
  has_cwe %>% 
    filter(cna != "mitre",
           published > (curdate - days(180)),
           published <= curdate) %>% 
    summarize(x = sum(cwe), n=n(), pct = x/n, date = curdate)
})

allplot <- fullplot %>% mutate(src = "All CNAs") %>% 
  bind_rows(fullplot2 %>% mutate(src = "Without MITRE"))
